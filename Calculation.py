...
####################################################################################
# def conductivity(h_omega, CornerState_index, EdgeState_index, BulkState_index):
@njit
def conductivity(h_omega):
    """calculate sigma"""
    # 初始化数值
    sigma = 0
    sigma_B_B = 0
    ...
    for n in range(len(vectors)):  # eigenvectors有多长就取多长
        ...
        aA = np.dot(vectors[n].T, V_y)  # 计算aA = <a|A
        Aa = np.dot(V_y, vectors[n])  # 计算Aa = |A|a>
...
# plt.savefig(mkpath+'/sigma_yy.eps')  # 保存为eps文件
