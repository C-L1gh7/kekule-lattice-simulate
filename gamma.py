...
plt.plot(x1, y1, color= '#F553DA', linestyle='-', lw=0.8, label = r'$\Gamma = 0.0005$')
plt.plot(x2, y2, color= '#53A8F5', linestyle='-.', lw=0.8, label = r'$\Gamma = 0.001$')
plt.plot(x3, y3, color= '#756372',linestyle=':', lw=0.8, label = r'$\Gamma = 0.005$')
plt.plot(x4, y4, color = '#F5B753', linestyle='--', lw=0.8, label = r'$\Gamma = 0.1$')
plt.plot(x5, y5, color = '#79F553', linestyle='-.', lw=0.8, label = r'$\Gamma = 0.2$')
plt.plot(x6, y6, color = '#A08C6C', linestyle=':', lw=0.8, label = r'$\Gamma = 0.3$')

plt.legend(frameon=False) # 显示图例
plt.xlabel(r'$\hbar\omega\ [t]$', fontsize=14)
plt.ylabel(r'$\sigma_{xx}\ [{\tilde t}^2 e^2/\hbar]$', fontsize=14)
plt.savefig('result/picture/Gamma_sigma_xx_total.eps')  # 保存为eps文件
plt.clf
...
# plt.savefig('result/picture/Gamma_sigma_xx_all.eps')  # 保存为eps文件